...
# Genny Paola Rivera Becerra / Computacion Blanda / Ingenieria de Sitemas y Computacion   
...
def plot_models(x, y, models, fname, mx=None, ymax=None, xmin=None):
    ...
    if models:
        ...
        # Este ciclo lo que hace es que para cada modelo y estilo se utiliza una paleta de colores.
        ...
        for model, style, color in zip(models, linestyles, colors):
            ...
            # plt.legend: Lo que hace esta funcion, es que nos muestra la definicion de una leyenda que significa cada linea que se va
            # a dibujar en el modelo y la posicion que se quiere mostrar.
        plt.legend(["d=%i" % m.order for m in models], loc="upper left")

           # plt.autoscala: Muestra los valores que estan en los ejes, para mostrarlos se debe indicar cuales ejes y como se quieren mostrar 
           # por ello se saca la definicion (tight=True), lo que inidica que valores van a estar agrupados o apretados en los dos ejes.
    plt.autoscale(tight=True)
   # plt.ylim: Permiten hacer zoom o expandir el grafico o establece los rangos de los ejes para incluir el origen.
    plt.ylim(ymin=0)
    # plt.ylim(ymax=ymax): Permite establecer los valores maximos del eje y.
    if ymax:
        plt.ylim(ymax=ymax)
     # plt.xlim(xmin=xmin): Permite establecer los valores minimos del eje x.
    if xmin:
        plt.xlim(xmin=xmin)
   
     # plt.grid: dibuja líneas de división en la gráfica, la definicion (linestyle='-', color='0.75')
     # permite poner el color y el tipo de grafica que se quiere dibujar en este caso "linea solida". 
    plt.grid(True, linestyle='-', color='0.75')
    # plt.savefig: Guarda la grafica o figura poniendo como parámetro el nombre del fichero con su extensión en este caso (fname).
    plt.savefig(fname) 

# Primera mirada a los datos
# plot_models: llama los modelos de datos, con la definicion os.path.join unimos uno o más componentes de ruta de forma inteligente
# en este caso estaria en (CHART_DIR, "1400_01_01.png").
plot_models(x, y, None, os.path.join(CHART_DIR, "1400_01_01.png"))

# Crea y dibuja los modelos de datos
# Define los parametros del modelo de datos fp1, la definicion (np.polyfit) nos permite aproximar un conjunto de datos según el  procedimiento de mínimos cuadrados.
fp1, res1, rank1, sv1, rcond1 = np.polyfit(x, y, 1, full=True)
# Imprime en pantalla un mensaje de los parametros del modelo fp1.
print("Parámetros del modelo fp1: %s" % fp1)
# Imprime en pantalla un mensaje si el modelo fp1 presenta un error.
print("Error del modelo fp1:", res1)
# sp.poly1d: Calcula los nuevos puntos polinomiales para fp1.
f1 = sp.poly1d(fp1)

# Define los parametros del modelo de datos fp2, la definicion (np.polyfit) nos permite aproximar un conjunto de datos según el  procedimiento de mínimos cuadrados
fp2, res2, rank2, sv2, rcond2 = np.polyfit(x, y, 2, full=True)
# Imprime en pantalla un mensaje de los parametros del modelo fp2.
print("Parámetros del modelo fp2: %s" % fp2)
# Imprime en pantalla un mensaje si el modelo fp2 presenta un error.
print("Error del modelo fp2:", res2)
# sp.poly1d: Calcula los nuevos puntos polinomiales para fp2.
f2 = sp.poly1d(fp2)

# sp.poly1d: Calcula los nuevos puntos polinomiales (f3, f10, f100) y realiza un ajuste polinomial de minimos cuadrados.
f3 = sp.poly1d(np.polyfit(x, y, 3))
f10 = sp.poly1d(np.polyfit(x, y, 10))
f100 = sp.poly1d(np.polyfit(x, y, 100))

# Se grafican los modelos
# Con la definicion os.path.join unimos uno o más componentes de ruta de forma inteligente.
#plot_models: Convierte un modelo a formato de puntos y lo guarda en un archivo
#path.join: Se utiliza para que los separadores de carpetas estén en los lugares correctos
...
# de inflexión
# Define los puntos de inflexion a calcular.
#
inflexion = 3.5 * 7 * 24
xa = x[:int(inflexion)]
ya = y[:int(inflexion)]
...
#sp.poly1d: Una clase de conveniencia, utilizada para encapsular operaciones "naturales" en polinomios para que dichas operaciones puedan tomar su forma habitual en el código
...
# y se guarda en una ruta inteligente.
# plot_models: Crea gráficos a partir de modelos de regresión, ya sean estimaciones (como los llamados gráficos de bosque o de puntos) o efectos marginales.
...
#Se crean nuevo modelos (fb1, fb2, fb3, fb10, fb100), con la diferencia de que no toman todos los datos del archivo, si no que toma los datos apartir del punto de #inflexión y en un orden aleatorio 
fb1 = fb
